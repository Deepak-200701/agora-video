...
const JoinCall = () => {
    ...
    useEffect(() => {
        const client = AgoraRTC.createClient({ mode: 'rtc', codec: 'vp8' });
        client.enableAudioVolumeIndicator(200, 3, true);
        clientRef.current = client;
        ...
        client.on('user-published', async (user, mediaType) => {
            ...
            if (mediaType === 'video' && user.videoTrack) {
                ...
                remoteVideoContainer.className =
                    'w-64 h-48 bg-gray-800 rounded-lg flex items-center justify-center text-white transition-all duration-200 transform scale-75';
        ...
        client.on('volume-indicator', (volumes) => {
            if (volumes.length > 0) {
                ...
                Object.keys(remoteContainerRef.current).forEach(uid => {
                    ...
                    if (uid == active.uid.toString()) {
                        el.className =
                            'w-96 h-64 bg-gray-900 rounded-lg flex items-center justify-center text-white transition-all duration-200 transform scale-125 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 border-4 border-green-400';
                    } else {
                        el.className =
                            'w-64 h-48 bg-gray-800 rounded-lg flex items-center justify-center text-white transition-all duration-200 transform scale-75 relative';
                    }
                });
            }
        });
    ...
    const fetchToken = async () => {
        const { data } = await axios.post("/api/auth/token", { channelName: channel });
        return data.token;
    };
    ...
    return (
        <>
            <div className="flex flex-col h-screen bg-gray-800">
                {/* Video Area */}
                <div className="flex-1 flex flex-col items-center gap-6 p-4 overflow-auto relative">
                    <p className="text-white text-lg mb-4">Active Speaker: {activeSpeaker}</p>
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div
                            id="remote-users-container"
                            className="flex flex-wrap justify-center items-center gap-6 mt-4 max-h-[300px] overflow-y-auto relative w-full"
                        >
                            {/* Remote user video containers will be injected here */}
                        </div>
                    </div>
                </div>
